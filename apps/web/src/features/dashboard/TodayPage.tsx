    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
      <Section title="今日焦点">
        <Card>
          <CardContent className="pt-5">
            ...
            </button>)}</div> : <EmptyState title="今天没有待办" action={<Button variant="outline" onClick={() => navigate("/app/execution")}>打开计划</Button>} />}
          </CardContent>
        </Card>
      </Section>
      ...
      <Section title="今日节奏">
        <Card><CardContent className="space-y-5 pt-5">
          ...
          <div><div className="eyebrow">最近复盘</div><div className="mt-2 text-sm font-medium">{latestReview ? text(latestReview, "bestThing", "已完成复盘") : "今天还没有复盘"}</div>{latestReview ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{text(latestReview, "tomorrowPriority", "查看复盘详情")}</p> : null}</div>
          <Button variant="outline" className="w-full" onClick={() => navigate("/app/review")}>打开复盘</Button>
        </CardContent></Card>
      </Section>
    </div>

    <Section className="mt-6" title="7 天行动趋势">
