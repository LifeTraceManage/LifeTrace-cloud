...
function writeDraftMeta(cacheKey: string, dirty: boolean) {
  try {
    ...
    // Browser storage may be unavailable; Cloud autosave remains authoritative.
  }
}
      ...
      hint: {
        ...
        extend: [{
          ...
          hint(keyword) {
            ...
            return wikiSuggestionsRef.current
              .filter((item) => {
              ...
              .slice(0, 8)
              .map((item) => ({
                html: `<span>${escapeHintHtml(item.title)}</span>${item.aliases?.length ? ` <small>${escapeHintHtml(item.aliases.join(" · "))}</small>` : ""}`,
                value: `[[${item.title}]]`,
              }));
          },
        }],
