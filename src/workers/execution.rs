...
async fn acquire_lease(state: &AppState, owner: Uuid) -> Result<bool, sqlx::Error> {
    let acquired = sqlx::query_scalar::<_, bool>(
        ...
        VALUES($1, $2, datetime('now', '+' || $3 || ' seconds'), CURRENT_TIMESTAMP)
...
async fn fire_due_reminders(state: &AppState) -> Result<usize, sqlx::Error> {
    let rows = sqlx::query(
        ...
        WHERE entity_type='execution.reminder'
          ...
          AND datetime(COALESCE(NULLIF(payload->>'snoozedUntil',''), payload->>'triggerAt')) <= CURRENT_TIMESTAMP
        ORDER BY datetime(COALESCE(NULLIF(payload->>'snoozedUntil',''), payload->>'triggerAt'))
        LIMIT $1
        "#,
    )
    .bind(BATCH_LIMIT)
