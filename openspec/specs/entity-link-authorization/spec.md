# Entity Link Authorization Specification

## Purpose
Defines least-privilege authorization for generic `entity.link` synchronization in LifeTrace Cloud while preserving existing identity/account authorization boundaries.

## Requirements

### Requirement: Dedicated link scopes
The Cloud MUST authorize generic `entity.link` synchronization through dedicated link scopes rather than account write authority.
...
The dedicated Assets application MUST be eligible for link read/write scopes while remaining ineligible for account write and unrelated product write scopes.
