...
fn execution_payload(entity_id: &str, user_id: &str, fields: Value) -> Value {
    let mut object = fields
        .as_object()
        .cloned()
        .expect("execution fields object");
    object.insert(
        "meta".to_owned(),
        json!({
            "id": entity_id,
...
async fn execution_two_devices_create_update_delete_and_tombstone() {
    ...
    let initial = vec![
        execution_change(
            ...
            0,
            "upsert",
            json!({"title":"Offline task","status":"todo","priority":"normal"}),
        ),
        execution_change(
            user,
            "exec-core-calendar-c1",
            "execution.calendar_event",
            "event-core",
            0,
            "upsert",
            json!({"title":"Focus block","isAllDay":false,"startAt":"2026-08-09T02:00:00Z","endAt":"2026-08-09T03:00:00Z","status":"scheduled"}),
        ),
        execution_change(
            user,
            "exec-core-waiting-c1",
            "execution.waiting_item",
            "waiting-core",
            0,
            "upsert",
            json!({"title":"Wait for reply","status":"open","waitingFor":"Alice"}),
        ),
        execution_change(
            user,
            "exec-core-memo-c1",
        ...
        execution_change(
            ...
            0,
            "upsert",
            json!({"subjectType":"task","subjectId":"task-core","triggerAt":"2026-08-10T00:00:00Z","status":"scheduled","fireKey":"task-core@2026-08-10"}),
        ),
    ];
    let (status, push) = send(
        ...
        "/api/v1/sync/push",
        TOKEN_A,
        execution_push_request("execution-device-a", initial),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    assert!(push["results"]
    ...
    for expected in [
        ...
        assert!(changes
            .iter()
            .any(|change| change["entityType"] == expected));
    }
    ...
    let (status, update) = send(
        ...
        execution_push_request(
            ...
            vec![execution_change(
                ...
                1,
                "upsert",
                json!({"title":"Task updated on A","status":"in_progress","priority":"high"}),
            )],
        ),
    )
    .await;
    ...
    let (status, update_pull) = execution_pull(
        app_b.clone(),
        TOKEN_A,
        "execution-device-b",
        Some(&cursor_after_create),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    ...
    assert_eq!(
        update_pull["changes"][0]["payload"]["title"],
        "Task updated on A"
    );
    ...
    let (status, delete_pull) = execution_pull(
        app_b,
        TOKEN_A,
        "execution-device-b",
        Some(&cursor_after_update),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
...
async fn execution_duplicate_and_domain_conflicts_are_not_silent() {
    ...
    let (_, task_create) = send(
        ...
        execution_push_request(
            ...
            vec![execution_change(
                user,
                "exec-task-conflict-c1",
                "execution.task",
                "task-conflict",
                0,
                "upsert",
                json!({"title":"Competing state","status":"todo","priority":"normal"}),
            )],
        ),
    )
    .await;
    ...
    let (_, cancelled) = send(app_b.clone(), Method::POST, "/api/v1/sync/push", TOKEN_A, execution_push_request("execution-conflict-b", vec![execution_change(user, "exec-task-conflict-c3", "execution.task", "task-conflict", 1, "upsert", json!({"title":"Competing state","status":"cancelled","priority":"normal","cancelledAt":"2026-08-09T01:00:00Z"}))])).await;
    assert_eq!(cancelled["results"][0]["status"], "conflict");
    assert_eq!(cancelled["results"][0]["serverEntity"]["status"], "done");

    let (_, recurrence_create) = send(
        app_a.clone(),
        Method::POST,
        "/api/v1/sync/push",
        TOKEN_A,
        execution_push_request(
            ...
            vec![execution_change(
                ...
                0,
                "upsert",
                json!({"frequency":"daily","intervalValue":1}),
            )],
        ),
    )
    .await;
    assert_eq!(recurrence_create["results"][0]["status"], "accepted");
    let (_, recurrence_a) = send(
        app_a,
        Method::POST,
        "/api/v1/sync/push",
        TOKEN_A,
        execution_push_request(
            ...
            vec![execution_change(
                ...
                1,
                "upsert",
                json!({"frequency":"daily","intervalValue":2}),
            )],
        ),
    )
    .await;
    assert_eq!(recurrence_a["results"][0]["status"], "accepted");
    let (_, recurrence_b) = send(
        app_b,
        Method::POST,
        "/api/v1/sync/push",
        TOKEN_A,
        execution_push_request(
            ...
            vec![execution_change(
                ...
                1,
                "upsert",
                json!({"frequency":"weekly","intervalValue":1,"weekdaysJson":"[1,3,5]"}),
            )],
        ),
    )
    .await;
    ...
    assert_eq!(
        recurrence_b["results"][0]["serverEntity"]["intervalValue"],
        2
    );
}
